...
mod tests {
    ...
    fn test_encode_single_byte() {
        // 测试单字节编码的边界值
        assert_eq!(encode(0), vec![0x00], "0 应编码为 [0x00]");
        assert_eq!(encode(1), vec![0x01], "1 应编码为 [0x01]");
        assert_eq!(encode(127), vec![0x7F], "127 应编码为 [0x7F]");

        // 测试一些随机单字节值
        assert_eq!(encode(42), vec![0x2A], "42 应编码为 [0x2A]");
        assert_eq!(encode(100), vec![0x64], "100 应编码为 [0x64]");
    }
    ...
    fn test_encode_multi_byte() {
        // 测试边界：第一个需要 2 字节的值
        assert_eq!(encode(128), vec![0x80, 0x01], "128 应编码为 [0x80, 0x01]");
        assert_eq!(encode(255), vec![0xFF, 0x01], "255 应编码为 [0xFF, 0x01]");
        assert_eq!(encode(300), vec![0xAC, 0x02], "300 应编码为 [0xAC, 0x02]");
    ...
    fn test_decode_single_byte() {
        assert_eq!(decode(&[0x00]), Ok(0), "[0x00] 应解码为 0");
        assert_eq!(decode(&[0x01]), Ok(1), "[0x01] 应解码为 1");
        assert_eq!(decode(&[0x7F]), Ok(127), "[0x7F] 应解码为 127");
        assert_eq!(decode(&[0x2A]), Ok(42), "[0x2A] 应解码为 42");
        assert_eq!(decode(&[0x64]), Ok(100), "[0x64] 应解码为 100");
    }

    /// 测试解码多字节值
    #[test]
    fn test_decode_multi_byte() {
        assert_eq!(decode(&[0x80, 0x01]), Ok(128), "2 字节的 128");
        assert_eq!(decode(&[0xFF, 0x01]), Ok(255), "2 字节的 255");
        assert_eq!(decode(&[0xAC, 0x02]), Ok(300), "2 字节的 300");
        assert_eq!(decode(&[0x80, 0x80, 0x01]), Ok(16384), "3 字节的 16384");
        assert_eq!(decode(&[0xFF, 0x7F]), Ok(16383), "2 字节最大值 (16383)");
    }
    ...
    fn test_decode_large_values() {
        ...
        // 测试 2^63 - 1
        assert_eq!(
            decode(&[0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F]),
    ...
    fn test_decode_incomplete() {
        // 设置了延续位的单字节
        assert_eq!(decode(&[0x80]), Err("Incomplete"), "单字节延续");
        ...
        // 空切片
        assert_eq!(decode(&[]), Err("Incomplete"), "空切片");
    }
    ...
    fn test_decode_overflow() {
        ...
        assert_eq!(decode(&max_valid), Ok(u64::MAX), "有效的 10 字节 varint");
    ...
    fn test_encode_minimal() {
        // 127 应为 1 字节，不是 2
        assert_eq!(encode(127).len(), 1, "127 应编码为 1 字节");

        // 128 应为 2 字节，不是更多
        assert_eq!(encode(128).len(), 2, "128 应编码为 2 字节");

        // 检查更大的值：2^14-1 = 16383 应为 2 字节
        assert_eq!(encode(16383).len(), 2, "16383 应编码为 2 字节");

        // 2^14 = 16384 应为 3 字节
        assert_eq!(encode(16384).len(), 3, "16384 应编码为 3 字节");
    }
