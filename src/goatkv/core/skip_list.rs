    ...
    K: UserKey,
{
    ...
    pub fn insert(&mut self, key: K, value: Bytes) -> Option<&Bytes> {
        ...
        if height > self.max_height {
            for prev_i in prev.iter_mut().take(height).skip(self.max_height) {
                *prev_i = Some(self.head);
            }
            self.max_height = height;
        }

        // 插入新节点到各层
        for (i, prev_i) in prev.iter_mut().enumerate().take(height) {
            if let Some(mut prev_node) = *prev_i {
...
mod tests {
    ...
    fn test_range() {
        ...
        for (i, key) in range.iter().enumerate().take(10) {
            assert_eq!(key, format!("{:02}", 20 + i).as_bytes());
        }
    }
