...
impl Shard {
    ...
    /// 创建新的 Shard，使用默认数据目录（当前目录下的 goatdb_data）
    pub fn new() -> Self {
        let options = KvEngineOptions::default();
        Self::new_with_options(options).expect("Failed to create Shard with default options")
    }

    /// 创建新的 Shard，使用指定的配置选项
    ///
    /// # 参数
...
impl Shard {
    #[cfg(test)]
    pub(crate) fn is_mem_state_empty(&self) -> bool {
        let state = self.lsm_state.read().unwrap();
        state.mem_table.is_empty() && state.immutable_mem_tables.is_empty()
    }
